...
\subsubsection*{Children}
\children{
  \child{expression}{A regular expression.}{Occurs 0 or more times.}{expression}
...
\subsubsection*{Children}
\children{
  \child{weightset}{The weight set.}{Occurs 1 time, is required.}{weightset}
  \child{labelSet}{The alphabet.}{Occurs 1 time, is required.}{labelSet}
}
...
\attrs{
  \attr{type}{Required, Pivot}{token}{none}{``unit'', ``free'', ``product''}
       {Type of the labelSet.}
}

\subsubsection{Unit Label Set}

When \xattr{type} is set to ``unit'', it is equivalent to the lack of
label sets. It enables the possibility to describe valued graphs within the
same format.
...
free labelSet and inherits the following new
...
the \xtag{labelSet} inherits the following new attributes and children.
...
\paragraph*{Children}
\children{
  \child{labelGen}{A label set generator.}{Occurs 1 or more times.}{labelGen}
}

\paragraph{Free label set with ``tuple'' generators}
...
\paragraph*{Children}
\children{
  \child{genSort}{List of sort of generator for each ``free'' label set.}
  {Occurs 1 time, is required.}{genSort}
  \child{labelGen}{A label set generator.}{Occurs 1 or more times.}{labelGen}
}

\subsubsection{Product Label Set}

When \xattr{type} is set to ``product'', the \xtag{labelSet} describes a
product of free label sets and inherits the following new attributes and children.
...
\subsubsection*{Children}
\children{
  \child{labelSet}{A \texttt{free} label set.}{Occurs \xval{prodDim} times.}{free label set}
}
...
label set with ``tuple'' generators.
...
When \xattr{genDescript} is set to ``enum'', the \xtag{labelGen} inherits
the following new attributes:

\paragraph{``enum'' generator of ``simple'' sort}

Describes a generator of label set when its \xattr{genDescript} is set to
...
If used within a \xtag{labelSet}, should also be \emph{Unique}.
...
Describes a generator of labelSet when its \xattr{genDescript} is set to
...
\paragraph*{Children}
\children{
  \child{monCompGen}{A ``tuple'' label set generator.}
...
\xval{k} being the position of the element in the list within \xtag{labelGen}.
...
When \xattr{type} is set to ``numerical'', \xtag{weightSet} describes a
numerical weight set and inherits the following new attributes and children.
\example{automatonA1}{6-8}

\subsubsection*{Attributes}
\attrs{
  \attr{set}{Required}{token}{none}{``B'', ``N'', ``Z'', ``Q'', ``R'', ``C''}
       {The set on which is described the weight set.}
...
When \xattr{type} is set to ``series'', the \xtag{weightSet} describes a
series weight set and inherits the following new attributes and children.
...
\children{
  ...
  \child{typedExpression}{The regular expression's body.}
  {Occurs 1 time.}{typedExpression}
}
...
Represents a label set element, which is a concatenation of label set generators.
...
\paragraph*{Children}
\children{
  \child{labelGen}{A label set Generator.}{Occurs as many time as wanted.}{labelGen}
}

\paragraph{On a product Label Set.}

Represents a label set element on a product label set.
...
\paragraph*{Children}
\children{
  \item \xtag{monElmt} or \xtag{one}  A label set element or the identity of
    a \texttt{free} label set. Occurs \xval{prodDim} times.
...
Represents the identity series or the identity symbol of a \texttt{free} label set.
...
Represents the weight of an expression with a numerical weightset.
...
\subsubsection*{Children}
\children{
  \RegExpBody{Typed regular expression taken into the weight set.}
       {Occurs 1 time.}
}
