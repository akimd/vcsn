...
\begin{itemize}
\item \emph{token}\\Is used for a string which takes only a couple of
  already defined values, listed in the \emph{Values} column.
\item \emph{ID}\\Is used for a string which represents an identifier,
  that can be used many times in the document to refer to the unique
  same ``concept''.
\item \emph{generator}\\Is used for a string which only takes a couple
  of values (implicitly) defined in the XML document.
\item \emph{weight}\\Is used for a string which only takes a couple of
  values (implicitly) defined in the XML document.
\item \emph{URI}\\A Uniform Resource Identifier (URI), is a compact
...
used within a \xtag{monElmt}, should also be \emph{Valid}.
\paragraph*{Children}
None.
...
\paragraph*{Children}
\children{
  \child{monCompGen}{A ``tuple'' label set generator. Occurs
    \xval{genDim} times.}{monCompGen}.
}
...
\attrs{
  \attr{set}{Required}{token}{none}{``B'', ``N'', ``Z'', ``Q'', ``R'',
    ``C''} {The set on which is described the weight set.}
  \attr{operation}{Required}{token, string}{none}{``classical'',
    ``minPlus'', ``maxPlus''…}{Set the operations to work with into the
...
Represents a \emph{labelSet} element, which is a concatenation of
\emph{labelSet} generators.

\paragraph{On a free Label Set}
\label{monElmtFree}

Represents a \emph{labelSet} element on a free \emph{labelSet}.
...
\paragraph*{Children}
\children{
  \child{labelGen}{A \emph{labelSet} Generator. Occurs as many time as
    wanted.}{labelGen}.
}

\paragraph{On a product Label Set.}

Represents a \emph{labelSet} element on a product \emph{labelSet}.
...
\paragraph*{Children}
\children{
\item \xtag{monElmt} or \xtag{one} A \emph{labelSet} element or the identity
  of a \texttt{free} \emph{labelSet}. Occurs \xval{prodDim} times.  See
  \autoref{monElmtFree} and \autoref{one} for more details.
}
...
Represents the weight of an expression with a numerical \emph{weightSet}.
...
Represents the weight of an expression with a series \emph{weightSet}.
...
\attrs{
  ...
  \attr{key}{}{integer}{0}{Positive integer}{Possible key of the
    state.}
