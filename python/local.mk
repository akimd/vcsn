...
from_odyn_gen =				\
  %D%/odyn.hh %D%/odyn.cc
CLEANFILES += %D%/odyn.stamp $(from_odyn_gen)
%D%/odyn.stamp: $(odyn_gen) vcsn/dyn/algos.json
	$(AM_V_GEN)$(mkdir_p) $(@D)
	$(AM_V_at)rm -f $@ $@.tmp
	$(AM_V_at)echo '$@ rebuilt because of: $?' >$@.tmp
	$(AM_V_at)$(PYTHON) $(srcdir)/$(odyn_gen)	\
	  --header %D%/odyn.hh.tmp			\
	  --output %D%/odyn.cc.tmp			\
	  vcsn/dyn/algos.json
...
$(from_odyn_gen): %D%/odyn.stamp
	@if test ! -f $@; then			\
	  rm -f $<;				\
	  $(MAKE) $(AM_MAKEFLAGS) $<;		\
	fi

BUILT_SOURCES += $(from_odyn_gen)
...
%C%_vcsn_cxx_la_SOURCES = $(from_odyn_gen) %D%/vcsn_cxx.cc
