...
XFAIL(lambda: vcsn.context("lal(a), b_z"),
  ...
  while reading context: "lal(a), b_z"''')
...
check(r'lal(ab), b',     r'letterset<char_letters(ab)>, b')
check(r'lal(a-kv-z), b', r'letterset<char_letters(abcdefghijkvwxyz)>, b')
check(r'lal(-0-9), b',   r'letterset<char_letters(\-0123456789)>, b')
check(r'lal(<>[]{}), b', r'letterset<char_letters(<>[]{})>, b')
check(r'lal(\\\-\)), b', r'letterset<char_letters(\)\-\\)>, b')
check(r'lal(\--\-), b',  r'letterset<char_letters(\-)>, b')

# Check that we cover all the possible chars.  Unfortunately as this
# is not valid UTF-8, Python will refuse to display it.  Usinsg
...
check('lal(ab), q', 'letterset<char_letters(ab)>, q')
...
check('expressionset<lal(ab), b>, b', 'expressionset<letterset<char_letters(ab)>, b>, b')
...
check('lat<lal(a),lal(a)>,b', 'lat<letterset<char_letters(a)>, letterset<char_letters(a)>>, b')
check('lal(ab), lat<b, z>', 'letterset<char_letters(ab)>, lat<b, z>')
