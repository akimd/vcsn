...
check(vcsn.context('[ab] -> z').expression('<2>a*<3>', 'associative'),
r'''digraph
{
  vcsn_context = "[ab]? → ℤ"
  rankdir = LR
...
# properly handled.
check(r'(?@[a] -> expressionset<[xyz] -> q>)<x>a(<y>\e+<z>a)',
r'''digraph
{
  vcsn_context = "[a]? → RatE[[xyz]? → ℚ]"
  rankdir = LR
  edge [arrowhead = vee, arrowsize = .6]
  {
    node [shape = point, width = 0]
